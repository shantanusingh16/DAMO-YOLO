    ...
    def forward(self, x):
        ...
        for i, f in enumerate(self.features):
            ...
            if i in self.out_indices:
                outs.append(x)
        # assert(len(outs) == 4) # Disabled since we may want to use less than 4 outputs
        return outs


def repvit_m0_6(pretrained=False, num_classes = 1000, distillation=False, init_cfg=None, out_indices=[], **kwargs):
    """
...
def load_repvit_net(backbone_cfg):
    model_type = backbone_cfg.pop('model_type')
    init_cfg = backbone_cfg.pop('init_cfg')
    out_indices = backbone_cfg.pop('out_indices')
    
    if model_type == "repvit_m0_6":
        model = repvit_m0_6(init_cfg=init_cfg, out_indices=out_indices)
    elif model_type == "repvit_m0_9":
        model = repvit_m0_9(init_cfg=init_cfg, out_indices=out_indices)
    elif model_type == "repvit_m1_0":
        model = repvit_m1_0(init_cfg=init_cfg, out_indices=out_indices)
    elif model_type == "repvit_m1_1":
        model = repvit_m1_1(init_cfg=init_cfg, out_indices=out_indices)
    elif model_type == "repvit_m1_5":
        model = repvit_m1_5(init_cfg=init_cfg, out_indices=out_indices)
    elif model_type == "repvit_m2_3":
        model = repvit_m2_3(init_cfg=init_cfg, out_indices=out_indices)
    else:
